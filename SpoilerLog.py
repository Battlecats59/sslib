...
def write(
    ...
    *,
    ...
    with_regions = [
        ...
        for loc, item in item_locations.items()
    ...
    for hintloc, hint_stone in hints.items():
        if isinstance(hint_stone, GossipStoneHintWrapper):
            ...
            file.write(
                f"  {norm(hintloc) + ':':{max_hintstone_name_length}} {str(hint_stone)}\n"
            )
...
def dump_json(
    ...
    *,
    ...
    if options["no-spoiler-log"]:
        return spoiler_log
    spoiler_log["starting-items"] = sorted(starting_items)
    ...
    spoiler_log["item-locations"] = item_locations
